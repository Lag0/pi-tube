...
describe("CLI transcription integration", () => {
  test("executes deepgram provider with stable output markers", () => {
    ...
    expect(stdout).toContain("[TRANSCRIPTION_RESOLVED] provider=deepgram");
    expect(stdout).toContain("transcript=hello deepgram");
  });
  ...
  test("supports groq provider switching and language preference", () => {
    ...
    const stdout = result.stdout.toString();
    expect(result.exitCode).toBe(0);
    expect(stdout).toContain("[TRANSCRIPTION_RESOLVED] provider=groq");
  ...
  test("uses env provider fallback when --provider is omitted", () => {
    ...
    expect(result.exitCode).toBe(0);
    expect(result.stdout.toString()).toContain("provider=groq");
  });
  ...
  test("prioritizes --provider over env provider fallback", () => {
    ...
    expect(stdout).toContain("transcript=cli wins");
  });
